...
def main():
    ...
    L += ["## 1. 方法論（為什麼可以相信這些數字）", "",
          ...
          "6. **市場代理對照**：另外訓練一個只用「莊家一定會定價的資訊」的模型，"
          "看完整模型有沒有贏過它。這是沒有真實賠率時最接近真相的檢驗。",
          "7. **球場校正**：用「該球場自己的歷史大分率」當基準，"
          "把莊家一定會定價的球場效應扣掉再看優勢還剩多少。",
          "8. **跨球季驗證**：一季挖掘、另一季驗證。球員陣容都換了還撐得住，"
          "才可能是真規律。", "",
    ...
    if MR:
        L += ["## 4. 得分期望值模型（主引擎）", "",
              ...
              "- 訓練集併入 2024+2025 兩個過去球季（過去的球季不會洩漏未來資訊）："
              "平均 AUC +0.011、27/42 個盤口進步，兩階段驗證 ROI 從 −2.6% 變 +0.3%",
    ...
    if AB:
        ...
        L += ["## 5.5 特徵族群消融實驗：這些細分項真的有用嗎", "",
              ...
              "3. **相對最不可取代的是球場與天氣**（1.8σ，對大小分 AUC 影響也最大）：拿掉它 MAE 變差 "
              ...
              "5b. **MAE 好不等於能賺**：把特徵砍到只剩 16 欄「莊家一定知道」的資訊，"
              "樣本外得分 MAE 反而更好（2.492 vs 2.501），但兩階段驗證的 ROI 從 "
              "+0.3% 掉到 −1.4%。原因是 MAE 由不可預測的變異主導，"
              "而下注賺不賺看的是「機率分布的尾端排序」。所以生產模型維持完整特徵。", "",
    ...
    if PROF:
        L += ["### 30 隊一句話摘要", "",
              ...
              table(["球隊", "打擊 wOBA", "投球 wOBA", "特徵"],
                    [[v["zh"], v.get("bat_woba"), v.get("pit_woba"), v["summary"]]
                     for v in sorted(PROF.values(), key=lambda x: -(x.get("bat_woba") or 0))]), ""]
