...
執行方式：由工作排程在台灣時間 00:00-10:00 之間每 30 分鐘叫一次，
這支腳本自己判斷有沒有比賽進入更新窗口：
  T-60 分（±15 分，即 T-75~T-45）→ 第一次個別更新
  T-30 分（±15 分，即 T-45~T-15）→ 第二次個別更新（打線幾乎一定出來了）
兩個窗口連續覆蓋 T-75~T-15，所以 30 分鐘掃一次不會漏掉。
...
# (代號, 目標分鐘, 容差)
# 排程是每 30 分鐘掃一次，所以容差要 ±15 分，兩個窗口才會連續覆蓋
# T-75 ~ T-15 這一整段 —— 否則掃到 T-45 時兩邊都不符，下一次掃已經來不及。
WINDOWS = [("t60", 60, 15), ("t30", 30, 15)]
